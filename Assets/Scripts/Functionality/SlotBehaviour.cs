...
public class SlotBehaviour : MonoBehaviour
{
  ...
  [SerializeField]
  private Sprite[] symbolEleven;
  [SerializeField]
  private Sprite[] symbolNine;
  [SerializeField]
  private Sprite[] symbolTen;
  [SerializeField]
  ...
  private IEnumerator StopAutoSpinCoroutine()
  {
    ...
    if (!IsFreeSpin && (AutoSpinRoutine != null || tweenroutine != null))
    {
      StopCoroutine(AutoSpinRoutine);
      StopCoroutine(tweenroutine);
      tweenroutine = null;
  ...
  {
    Debug.Log($"[FREE SPIN] FreeSpin() called. Spins={spins}, IsFreeSpin={IsFreeSpin}");

    if (!IsFreeSpin)
    {
      Debug.Log("[FREE SPIN] Starting FreeSpin Coroutine");
      ...
      {
        Debug.Log("[FREE SPIN] Stopping previous coroutine");
  ...
  private IEnumerator FreeSpinCoroutine(int spinchances)
  {
    ...
    while (keepSpinning)
    {
      ...
      var respin = SocketManager.ResultData.payload.respinBonus;
      uiManager.FreeSpins = respin.respinRemaining;
      if (FSnum_text) FSnum_text.text = uiManager.FreeSpins.ToString();
      keepSpinning = respin.active && respin.respinRemaining > 0;
    }
    if (FSBoard_Object) FSBoard_Object.SetActive(false);
    if (WasAutoSpinOn)
    {
  ...
  private IEnumerator TweenRoutine()
  {
    ...
    BalanceTween?.Kill();
    if (Balance_text) Balance_text.text = SocketManager.ResultData.player.balance.ToString("F2");
    ...
    //   }
    // }
    if (SocketManager.ResultData.payload.respinBonus.triggered)
    {
      if (IsFreeSpin)
      {
        IsFreeSpin = false;
      ...
      FreeSpin(SocketManager.ResultData.payload.respinBonus.totalRounds);
  ...
  {
    AnimateNormalText(SocketManager.ResultData.payload.winAmount);
